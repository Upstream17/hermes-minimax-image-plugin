> **中文用户请看 [README.zh.md](README.zh.md)。** *(This README is in English. A Chinese version is available.)*
>
> 一行命令装好,MiniMax Token Plan 用户的 key 自动复用。

# Hermes MiniMax Image Plugin

A [Hermes Agent](https://github.com/NousResearch/hermes-agent) user plugin that adds
**MiniMax (China region) image generation** — `image-01` and `image-01-live` — as a
drop-in `image_generate` backend. The same `MINIMAX_CN_API_KEY` you use for chat
covers image generation on your Token Plan, so no separate billing is needed.
...
plugin adds a new implementation that hits the China-region MiniMax endpoint
(`https://api.minimaxi.com/v1/image_generation`) with a Bearer token.
...
- **Real API response shape is `data.image_urls`, not top-level
  ...
  parser in this plugin handles the real shape.
...
invalid for the China region. Make sure you grabbed a Token Plan key from
`platform.minimax.io` (not the global `minimax.io` key).
