...
`agent/image_gen_provider.py` 是 Hermes 的 `ImageGenProvider` ABC。Fal、xAI、OpenAI 这些后端都实现这个接口。这个 plugin 加了一个新实现,打的是 MiniMax 国内域的 `https://api.minimaxi.com/v1/image_generation` endpoint,带 Bearer token。
...
- 🤥 **MiniMax 官方文档写错了 response 格式**——它说字段是顶层的 `image_urls`,实际上是 `data.image_urls`。plugin 的 parser 写的是**真实**形态
...
**`MiniMax returned error: ...` 带 HTTP 401/403** — key 设了但不对国内域。重新去 `platform.minimax.io` 拿 Token Plan key(global `minimax.io` 的 key 不能用在国内 endpoint)。
